...
const TripDays = ({ onPrev, onNext }) => {
  const [tripDays, setTripDays] = useState(""); // State to track the trip days input
  const [showDateRangePicker, setShowDateRangePicker] = useState(false); // State to control showing DateRangePicker

  const handleNext = () => {
    if (isValidInput()) {
      setShowDateRangePicker(true); // Show the DateRangePicker component
    }
  };

  // Function to check if input is a valid number
  const isValidInput = () => {
    const parsedValue = parseInt(tripDays, 10);
    return !isNaN(parsedValue) && parsedValue > 0; // Check if the input is a valid positive number
  };

  const handleInputChange = (e) => {
    setTripDays(e.target.value); // Update tripDays state with the input value
  };
  ...
  if (showDateRangePicker) {
    return <DateRangePicker onPrev={onPrev} />;
  }
      ...
      <div className="mt-32 flex justify-center items-center">
        <div className="text-center">
          <h1 className="text-2xl md:text-3xl font-semibold">
            How many days trip are you planning?
          </h1>
        </div>
      </div>
      ...
      <div className="mt-6 md:mt-10 w-full px-4 flex items-center justify-center">
        <div className="relative w-full max-w-2xl">
          ...
          <input
            type="text"
            value={tripDays}
            placeholder="Trip Days. For eg. 7"
